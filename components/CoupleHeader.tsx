...
export default function CoupleHeader() {
  ...
  return (
    <div className="paper-card rounded-2xl px-6 py-5 mb-8 flex flex-col sm:flex-row items-center sm:items-stretch gap-6">
      <div className="flex items-center gap-4 flex-1 justify-center sm:justify-start">
        ...
        />
        <Heart size={20} className="fill-coral text-coral shrink-0" />
        <Avatar
      ...
      <div className="flex flex-col items-center justify-center border-t sm:border-t-0 sm:border-l border-charcoal/10 pt-4 sm:pt-0 sm:pl-6 min-w-[160px]">
        {days !== null ? (
          <>
            <span className="font-display text-3xl font-semibold text-ink leading-none">
              {days.toLocaleString("vi-VN")}
            </span>
            <span className="text-xs text-charcoal/60 font-mono tracking-wide mt-1">
              ngày bên nhau
            </span>
          </>
        ) : (
      ...
      <button
        ...
        className="self-center sm:self-start p-2 rounded-full hover:bg-charcoal/10 text-charcoal/60 hover:text-ink transition-colors"
      >
        <Pencil size={16} />
      </button>
...
function Avatar({
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div
        className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full ring-2 ring-offset-2 ring-offset-paper ${ringClass} overflow-hidden bg-lilac flex items-center justify-center shrink-0`}
      >
        {url ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={url} alt={name} className="w-full h-full object-cover" />
        ) : (
          <span className="font-display text-xl text-ink">{initial}</span>
        )}
      </div>
      <span className="text-xs font-medium text-charcoal/80 max-w-[80px] truncate">
        {name}
      </span>
    </div>
  );
